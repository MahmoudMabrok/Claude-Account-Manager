...
function generateScript(name) {
  const accountDir = configDir(name);
  const settingsPath = path.join(accountDir, 'settings.json');
  let syncSkills = true;
  let syncTools = true;
  ...
  if (fs.existsSync(settingsPath)) {
    try {
      ...
      if (settings.syncSkills === false) syncSkills = false;
      if (settings.syncTools === false) syncTools = false;
    } catch (e) { }
  }
  ...
  if (syncSkills) {
    syncLogic += `
# Sync global skills into this account
mkdir -p "\${accountDir}/skills"
if [ -d "$HOME/.claude/skills" ]; then
  for skill in "$HOME/.claude/skills/"*/; do
    [ -e "$skill" ] || continue
    skill_name=$(basename "$skill")
    ln -sf "$skill" "\${accountDir}/skills/$skill_name"
  done
fi
`;
  }
  ...
  if (syncTools) {
    syncLogic += `
# Sync global tools into this account
mkdir -p "\${accountDir}/tools"
if [ -d "$HOME/.claude/tools" ]; then
  for tool in "$HOME/.claude/tools/"*/; do
    [ -e "$tool" ] || continue
    tool_name=$(basename "$tool")
    ln -sf "$tool" "\${accountDir}/tools/$tool_name"
  done
fi
`;
  }
...
${syncLogic}
exec env CLAUDE_CONFIG_DIR="\${accountDir}" claude "$@"
`;
...
function getAccountInfo(name) {
  ...
  let info = { hasAuth: false, displayName: '', email: '', org: '', billingType: '', model: '', lastUsed: null, skillsCount: 0, toolsCount: 0, syncSkills: true, syncTools: true };
  ...
  if (fs.existsSync(settingsJson)) {
    try {
      ...
      info.syncSkills = settings.syncSkills !== false;
      info.syncTools = settings.syncTools !== false;
    } catch (e) { }
  }
...
app.post('/api/accounts', (req, res) => {
  const { name, syncSkills, syncTools } = req.body;
  ...
  const settings = {};
  if (DEFAULT_MODEL) settings.model = DEFAULT_MODEL;
  if (syncSkills !== undefined) settings.syncSkills = !!syncSkills;
  if (syncTools !== undefined) settings.syncTools = !!syncTools;
...
app.post('/api/accounts/:name/settings', (req, res) => {
  const { name } = req.params;
  const { model, syncSkills, syncTools } = req.body;
  const dir = configDir(name);
  const settingsPath = path.join(dir, 'settings.json');
  let settings = {};
  if (fs.existsSync(settingsPath)) {
  ...
  fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2));
  generateScript(name); // Regenerate script with new sync settings
  res.json({ ok: true });
});
...
function renderHTML(meta, lang, theme) {
  const t = {
    ar: {
      ...
      skills: 'المهارات',
      tools: 'الأدوات',
      syncSkills: 'مزامنة مهارات النظام',
      syncTools: 'مزامنة أدوات النظام',
    },
    en: {
      title: 'Claude Account Manager',
      ...
      skills: 'Skills',
      tools: 'Tools',
      syncSkills: 'Sync System Skills',
      syncTools: 'Sync System Tools',
    }
  };
  const s = t[lang] || t.ar;
      ...
      <div class="account" style="border-color: ${color}40">
        <div class="account-header">
          ...
          <div class="account-actions">
            ...
            <button onclick="openSettings('${name}', '${info.model || ''}', ${info.syncSkills}, ${info.syncTools})" class="btn btn-sm btn-settings" title="${s.settingsBtn}">${s.settingsBtn}</button>
        ...
        <div class="account-details">
          ...
          <div class="detail"><span class="detail-label">${s.skills}</span><span>${info.skillsCount}</span></div>
          <div class="detail"><span class="detail-label">${s.tools}</span><span>${info.toolsCount}</span></div>
...
<body>
  <div class="container">
    ...
    <div class="add-form">
      ...
      <div class="form-row" style="margin-top: 1rem; gap: 1rem;">
        <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; font-size: 0.85rem; color: ${textMuted};">
          <input type="checkbox" id="addSyncSkills" checked style="width: auto; padding: 0; margin: 0;"> <span>${s.syncSkills}</span>
        </label>
        <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; font-size: 0.85rem; color: ${textMuted};">
          <input type="checkbox" id="addSyncTools" checked style="width: auto; padding: 0; margin: 0;"> <span>${s.syncTools}</span>
        </label>
      </div>
    </div>
    ...
    <div class="modal">
      ...
      <div class="modal-row">
        <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
          <input type="checkbox" id="settingsSyncSkills" style="width: auto; padding: 0; margin: 0;"> <span>${s.syncSkills}</span>
        </label>
      </div>
      <div class="modal-row">
        <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
          <input type="checkbox" id="settingsSyncTools" style="width: auto; padding: 0; margin: 0;"> <span>${s.syncTools}</span>
        </label>
      </div>
      <div class="modal-row">
        <label>Color</label>
  ...
  <script>
    ...
    async function addAccount() {
      ...
      const syncSkills = document.getElementById('addSyncSkills').checked;
      const syncTools = document.getElementById('addSyncTools').checked;
      if (!name) return;
      const res = await fetch('/api/accounts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, syncSkills, syncTools })
      });
    ...
    function openSettings(name, currentModel, syncSkills, syncTools) {
      document.getElementById('settingsName').value = name;
      document.getElementById('settingsModel').value = currentModel;
      document.getElementById('settingsSyncSkills').checked = syncSkills;
      document.getElementById('settingsSyncTools').checked = syncTools;
      selectedColor = '';
    ...
    async function saveSettings() {
      ...
      const syncSkills = document.getElementById('settingsSyncSkills').checked;
      const syncTools = document.getElementById('settingsSyncTools').checked;

      await fetch('/api/accounts/' + name + '/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model, syncSkills, syncTools })
      });
